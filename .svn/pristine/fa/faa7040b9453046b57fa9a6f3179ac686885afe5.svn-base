...
SELECT_PROJECTSITEINSP_INFO = """SELECT 
									...
									A.CHECK_POSITION AS check_position,
...
class sqlProjectInspManage:
    ...
    def iPutInspInfo(self, userId, dataInfo):
        ...
        query += '"' + dataInfo["check_position"] + '", '
        query += '"' + dataInfo["insp_area"] + '", '
    ...
    def uUpdateInspInfo(self, dataInfo):
        ...
        query += 'CHECK_POSITION = "' + dataInfo["check_position"] + '", '
        query += 'INSP_AREA = "' + dataInfo["insp_area"] + '", '
    ...
    def sGetInspInfoList(self, params):
        ...
        if commUtilServ.dataCheck(params["search_content"]) != False:
            query += 'AND A.CONTENT = "' + params["search_content"] + '" '
