...
projectDetectionManageApi = Blueprint("projectDetectionManageApi", __name__)
...
# 검측 리스트 조회 API
@projectDetectionManageApi.route("/searchDetectionList", methods=["POST"])
    ...
    try:
        ...
        if jobResData == None:
            if (
                ...
                #################################### 검측 리스트를 조회 한다. ####################################
        resCd, msg, resDataList = servProjDeteMana.searchDetectionInfoList(
        ...
        #################################### 검측 리스트 수를 조회 한다. ####################################
        resCd, msg, resDataCnt = servProjDeteMana.searchDetectionInfoListCnt(
            loginUserInfo, loginUserInfo["authority_code"], jobResData, params
        )
        # Error 발생 시 에러 코드 리턴
        if resCd != 0:
