...
from projectMessageBoardManage.servProjectMessageBoardManage import (
    servProjectMessageBoardManage,
)

projectMessageBoardManageApi = api.namespace(
    "projectMessageBoardApi", description="게시판 관리"
)

logs = logManage()  # 사용자 관리 API 로그를 남기기 위한 객체 할당


@projectMessageBoardManageApi.route("/MessageBoardList/<cons_code>")
class projectMessageListManage(Resource):
    """Q&A 게시글 리스트 조회 api"""

    @api.doc(description="Q&A 게시글 리스트 조회")
    @api.expect(
        ...
        api.parser().add_argument(
            "category_keyword", type=str, location="querys", required=False
        ),
        api.parser().add_argument(
    ...
    def get(self, cons_code):
        ...
        servProjMBMana = servProjectMessageBoardManage()
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    ...
                    f"식별하지 못한 사용자가 게시글 리스트를 조회시도 하였습니다.",
            ...
            if loginUserInfo == None:
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"권한없는 사용자가 게시글 리스트를 조회시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
            ...
            servLogManage.post_log(
                ...
                f"{loginUserInfo['id']} 사용자가 게시글 리스트를 조회하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                msg
            ...
            servLogManage.post_log(
                ...
                f"{loginUserInfo['id']} 사용자가 게시글 리스트를 조회하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
...
@projectMessageBoardManageApi.route("/MessageBoard/<cons_code>")
class projectMessageManage(Resource):
    """Q&A 게시글 관리 api"""

    @api.doc(description="Q&A 게시글 작성")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"식별하지 못한 사용자가 게시글을 작성시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    "",
                    resCd,
                    msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_INFO,
                f"{loginUserInfo['id']} 사용자가 게시글을 작성하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_CRI,
                f"{loginUserInfo['id']} 사용자가 게시글을 작성하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
    ...
    @api.doc(description="Q&A 게시글 조회")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"식별하지 못한 사용자가 게시글을 조회시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    "",
                    resCd,
                    msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_INFO,
                f"{loginUserInfo['id']} 사용자가 게시글을 조회하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_CRI,
                f"{loginUserInfo['id']} 사용자가 게시글을 조회하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
    ...
    @api.doc(description="Q&A 게시글 수정")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"식별하지 못한 사용자가 게시글을 수정시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    "",
                    resCd,
                    msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_INFO,
                f"{loginUserInfo['id']} 사용자가 게시글을 수정하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_CRI,
                f"{loginUserInfo['id']} 사용자가 게시글을 수정하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
    ...
    @api.doc(description="Q&A 게시글 삭제")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"식별하지 못한 사용자가 게시글을 삭제시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    "",
                    resCd,
                    msg
            ...
            params = {"resCd": resCd, "cons_code": cons_code, "uuid": uuid, "msg": msg}
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_INFO,
                f"{loginUserInfo['id']} 사용자가 게시글을 삭제하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
            )
            return commServ.makeReturnMessage(resCd, msg, None)
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_CRI,
                f"{loginUserInfo['id']} 사용자가 게시글을 삭제하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
...
@projectMessageBoardManageApi.route("/MessageBoardLock/<cons_code>/<uuid>")
class projectMessageManage(Resource):
    """Q&A 게시글 검토관리 api"""

    @api.doc(description="Q&A 게시글 검토상태 변경")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"식별하지 못한 사용자가 게시글을 잠금시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    "",
                    resCd,
                    msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_INFO,
                f"{loginUserInfo['id']} 사용자가 게시글을 잠금하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_CRI,
                f"{loginUserInfo['id']} 사용자가 게시글을 잠금하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
...
@projectMessageBoardManageApi.route("/MessageBoardReply/<post_uuid>")
class projectMessageReplyManage(Resource):
    """게시판 댓글 관리 api"""

    @api.doc(description="Q&A 댓글 작성")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        api.parser().add_argument(
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"식별하지 못한 사용자가 게시글 댓글을 작성시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    "",
                    resCd,
                    msg
            ...
            if loginUserInfo == None:
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"권한없는 사용자가 게시글 댓글을 작성시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    loginUserInfo.get("id", "") if loginUserInfo else "",
                    resCd,
                    msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_INFO,
                f"{loginUserInfo['id']} 사용자가 게시글 댓글을 작성하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_CRI,
                f"{loginUserInfo['id']} 사용자가 게시글 댓글을 작성하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"식별하지 못한 사용자가 게시글 댓글을 조회시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    "",
                    resCd,
                    msg
            ...
            if loginUserInfo == None:
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"권한없는 사용자가 게시글 댓글을 조회시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    loginUserInfo.get("id", "") if loginUserInfo else "",
                    resCd,
                    msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_INFO,
                f"{loginUserInfo['id']} 사용자가 게시글 댓글을 조회하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_CRI,
                f"{loginUserInfo['id']} 사용자가 게시글 댓글을 조회하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"식별하지 못한 사용자가 게시글 댓글을 수정시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    "",
                    resCd,
                    msg
            ...
            if loginUserInfo == None:
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"권한없는 사용자가 게시글 댓글을 수정시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    loginUserInfo.get("id", "") if loginUserInfo else "",
                    resCd,
                    msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_INFO,
                f"{loginUserInfo['id']} 사용자가 게시글 댓글을 수정하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_CRI,
                f"{loginUserInfo['id']} 사용자가 게시글 댓글을 수정하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"식별하지 못한 사용자가 게시글 댓글을 삭제시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    "",
                    resCd,
                    msg
            ...
            if loginUserInfo == None:
                ...
                servLogManage.post_log(
                    procCode,
                    constants.LOG_LEVEL_CODE_WAR,
                    f"권한없는 사용자가 게시글 댓글을 삭제시도 하였습니다.",
                    json.dumps(params, ensure_ascii=False),
                    loginUserInfo.get("id", "") if loginUserInfo else "",
                    resCd,
                    msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_INFO,
                f"{loginUserInfo['id']} 사용자가 게시글 댓글을 삭제하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                msg
            ...
            servLogManage.post_log(
                procCode,
                constants.LOG_LEVEL_CODE_CRI,
                f"{loginUserInfo['id']} 사용자가 게시글 댓글을 삭제하던 중 에러가 발생하였습니다.",
                json.dumps(params, ensure_ascii=False),
                loginUserInfo["id"],
                resCd,
                str(e)
