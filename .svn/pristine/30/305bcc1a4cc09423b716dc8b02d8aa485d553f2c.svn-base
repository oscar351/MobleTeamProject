...
from projectRequestBoardManage.servProjectRequestBoardManage import (
    servProjectRequestBoardManage,
)

projectRequestBoardManageApi = api.namespace(
    "projectRequestBoardApi", description="의뢰게시판 관리"
)

logs = logManage()  # 사용자 관리 API 로그를 남기기 위한 객체 할당


@projectRequestBoardManageApi.route("/RequestBoardList/<cons_code>")
class projectRequestListManage(Resource):
    """의뢰 게시글 리스트 조회 api"""

    @api.doc(description="의뢰 게시글 리스트 조회")
    @api.expect(
    ...
    def get(self, cons_code):
        ...
        servProjRBMana = servProjectRequestBoardManage()
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"식별하지 못한 사용자가 의뢰서 리스트 조회를 시도하였습니다.",
            ...
            if loginUserInfo == None or (
                ...
                params.update({"cons_code": cons_code})
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"권한없는 사용자가 의뢰서 리스트 조회를 시도하였습니다.",
            ...
            resCd, msg, WRData = servProjRBMana.get_request_list(
                loginUserInfo["id"],
                cons_code,
            ...
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_INFO,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서 리스트 조회를 하였습니다.",
            ...
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_CRI,
                                    f"{loginUserInfo['id']} 사용자가 의뢰서 리스트 조회를 하던 중 에러가 발생하였습니다.",
...
@projectRequestBoardManageApi.route("/RequestBoard/<cons_code>")
class projectRequestManage(Resource):
    """의뢰 게시글 관리 api"""

    @api.doc(description="의뢰 게시글 작성")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
    ...
    def post(self, cons_code):
        ...
        servProjABMana = servProjectRequestBoardManage()
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                params.update({"cons_code": cons_code})
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"식별하지 못한 사용자가 의뢰서를 작성시도 하였습니다.",
                                    json.dumps(params, ensure_ascii=False), "", resCd, msg)
                result = commServ.makeReturnMessage(resCd, msg, None)
            ...
            resCd, msg, post_uuid = servProjABMana.post_request(
                cons_code,
                loginUserInfo["co_code"],
                loginUserInfo["id"],
                params["post_type"],
            ...
            params.update({"cons_code": cons_code})
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_INFO,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 작성하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, msg)
            ...
            params.update({"cons_code": cons_code})
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_CRI,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 작성하던 중 에러가 발생하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, str(e))
            result = commServ.makeReturnMessage(
                constants.REST_RESPONSE_CODE_DATAFAIL, str(e), None
            )
    ...
    def get(self, cons_code):
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"식별하지 못한 사용자가 의뢰서를 조회시도 하였습니다.",
                                    json.dumps(params, ensure_ascii=False), "", resCd, msg)
                result = commServ.makeReturnMessage(resCd, msg, None)
            ...
            resCd, msg, WRData = servProjABMana.get_request(
                loginUserInfo["id"], cons_code, loginUserInfo["co_code"], uuid
            )
            params = {"cons_code": cons_code, "uuid":uuid}
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_INFO,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 조회하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, msg)
            ...
            params = {"cons_code": cons_code, "uuid":uuid}
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_CRI,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 조회하던 중 에러가 발생하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, str(e))
            result = commServ.makeReturnMessage(
                constants.REST_RESPONSE_CODE_DATAFAIL, str(e), None
            )
    ...
    def put(self, cons_code):
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                params.update({"cons_code": cons_code})
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"식별하지 못한 사용자가 의뢰서를 수정시도 하였습니다.",
                                    json.dumps(params, ensure_ascii=False), loginUserInfo.get("id", "") if loginUserInfo else "", resCd, msg)
                logs.war(
                    procName,
                    os.path.basename(__file__),
            ...
            resCd, msg, _ = servProjABMana.put_request(
                cons_code,
                params["uuid"],
                params["post_type"],
                params["title"],
                params["content"],
                params["Request"] if "Request" in params else None,
                params["indexes"],
                request.files,
            )
            params.update({"cons_code": cons_code})
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_INFO,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 수정하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, msg)
            ...
            params.update({"cons_code": cons_code})
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_CRI,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 수정하던 중 에러가 발생하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, str(e))
            result = commServ.makeReturnMessage(
                constants.REST_RESPONSE_CODE_DATAFAIL, str(e), None
            )
    ...
    def delete(self, cons_code):
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                params = {"cons_code": cons_code, "uuid":uuid}
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"식별하지 못한 사용자가 의뢰서를 삭제시도 하였습니다.",
                                    json.dumps(params, ensure_ascii=False), loginUserInfo.get("id", "") if loginUserInfo else "", resCd, msg)
                result = commServ.makeReturnMessage(resCd, msg, None)
            ...
            resCd, msg, _ = servProjABMana.delete_request(cons_code, uuid)
            params = {"cons_code": cons_code, "uuid":uuid}
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_INFO,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 삭제하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, msg)
            return commServ.makeReturnMessage(resCd, msg, None)
            ...
            params = {"cons_code": cons_code, "uuid":uuid}
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_CRI,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 삭제하던 중 에러가 발생하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, str(e))
            result = commServ.makeReturnMessage(
                constants.REST_RESPONSE_CODE_DATAFAIL, str(e), None
            )
...
"""
@projectRequestBoardManageApi.route("/RequestBoardReply/<post_uuid>")
class projectMessageReplyManage(Resource):


    @api.doc(description="의뢰 댓글 작성")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        api.parser().add_argument(
    ...
    def post(self, post_uuid):
        ...
        try:
            ...
            resCd, msg, _ = servProjABMana.post_reply(
    ...
    def get(self, post_uuid):
        ...
        try:
            ...
            resCd, msg, WRData = servProjABMana.get_reply(post_uuid, parent_uuid)
            return commServ.makeReturnMessage(resCd, msg, WRData)
    ...
    def put(self, post_uuid):
        ...
        try:
            ...
            resCd, msg, _ = servProjABMana.put_reply(post_uuid, uuid, params["content"])
            return commServ.makeReturnMessage(resCd, msg, None)
    ...
    def delete(self, post_uuid):
        ...
        try:
            ...
            resCd, msg, _ = servProjABMana.del_reply(post_uuid, uuid)
            return commServ.makeReturnMessage(resCd, msg, None)
...
@projectRequestBoardManageApi.route(
    "/RequestBoardList/<cons_code>/<post_uuid>/drafted"
)
class projectRequestListManage(Resource):
    """의뢰 게시글 기안 api"""

    @api.doc(description="의뢰 게시글 결재 기안")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        api.parser().add_argument(
    ...
    def post(self, cons_code, post_uuid):
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"식별하지 못한 사용자가 의뢰서를 기안시도 하였습니다.",
                                    json.dumps(params, ensure_ascii=False), "", resCd, msg)
                logs.war(
                    procName,
                    os.path.basename(__file__),
            ...
            params = {"cons_code": cons_code, "uuid":post_uuid}
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_INFO,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 기안하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, msg)
            return commServ.makeReturnMessage(resCd, msg, None)
            ...
            params = {"cons_code": cons_code, "uuid":post_uuid}
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_CRI,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 기안하던 중 에러가 발생하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, str(e))
...
@projectRequestBoardManageApi.route(
    "/RequestBoardList/<cons_code>/<post_uuid>/withdraw"
)
class projectRequestListManage(Resource):
    """의뢰 게시글 철회 api"""

    @api.doc(description="의뢰 게시글 결재 철회")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        api.parser().add_argument(
    ...
    def post(self, cons_code, post_uuid):
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                params = {"cons_code": cons_code, "uuid":post_uuid}
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"식별하지 못한 사용자가 의뢰서를 철회시도 하였습니다.",
                                    json.dumps(params, ensure_ascii=False), "", resCd, msg)
                logs.war(
                    procName,
                    os.path.basename(__file__),
            ...
            params = {"cons_code": cons_code, "uuid":post_uuid}
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_INFO,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 철회하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, msg)
            return commServ.makeReturnMessage(resCd, msg, None)
            ...
            params = {"cons_code": cons_code, "uuid":post_uuid}
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_CRI,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 철회하던 중 에러가 발생하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, str(e))
            return commServ.makeReturnMessage(
                constants.REST_RESPONSE_CODE_DATAFAIL, str(e), None
            )
...
@projectRequestBoardManageApi.route(
    "/RequestBoardList/<cons_code>/<post_uuid>/approved"
)
class projectRequestListManage(Resource):
    """의뢰 게시글 결재 의뢰 api"""

    @api.doc(description="의뢰 게시글 결재 의뢰")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
    ...
    def post(self, cons_code, post_uuid):
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                ...
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"식별하지 못한 사용자가 의뢰서를 결재시도 하였습니다.",
                                    json.dumps(params, ensure_ascii=False), "", resCd, msg)
                logs.war(
                    procName,
                    os.path.basename(__file__),
            ...
            params.update({"cons_code": cons_code, "uuid":post_uuid})
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_INFO,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 결재하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, msg)
            return commServ.makeReturnMessage(resCd, msg, None)
            ...
            params.update({"cons_code": cons_code, "uuid":post_uuid})
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_CRI,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 결재하던 중 에러가 발생하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, str(e))
            return commServ.makeReturnMessage(
                constants.REST_RESPONSE_CODE_DATAFAIL, str(e), None
            )
...
@projectRequestBoardManageApi.route(
    "/RequestBoardList/<cons_code>/<post_uuid>/denied"
)
class projectRequestListManage(Resource):
    """의뢰 게시글 결재 거절 api"""

    @api.doc(description="의뢰 게시글 결재 거절")
    @api.expect(
        api.parser().add_argument("token", type=str, location="headers", required=True),
        api.parser().add_argument("sysCd", type=str, location="headers", required=True),
        api.parser().add_argument("remarks", type=str, location="json", required=True),
    ...
    def post(self, cons_code, post_uuid):
        ...
        try:
            ...
            if resCd != 0:  # DB 에러 발생 시
                params.update({"cons_code": cons_code, "uuid":post_uuid})
                servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_WAR,
                                    f"식별하지 못한 사용자가 의뢰서를 반려시도 하였습니다.",
                                    json.dumps(params, ensure_ascii=False), "", resCd, msg)
                logs.war(
                    procName,
                    os.path.basename(__file__),
            ...
            params.update({"cons_code": cons_code, "uuid":post_uuid})
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_INFO,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 반려하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, msg)
            return commServ.makeReturnMessage(resCd, msg, None)
            ...
            params.update({"cons_code": cons_code, "uuid":post_uuid})
            servLogManage.post_log(procCode, constants.LOG_LEVEL_CODE_CRI,
                                   f"{loginUserInfo['id']} 사용자가 의뢰서를 반려하던 중 에러가 발생하였습니다.",
                                   json.dumps(params, ensure_ascii=False), loginUserInfo['id'], resCd, str(e))
            return commServ.makeReturnMessage(
                constants.REST_RESPONSE_CODE_DATAFAIL, str(e), None
            )
